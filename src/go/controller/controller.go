...
func parseArguments(ctx *Context) State {
	...
	if filename == "" || username == "" || port == 0 {
		fmt.Fprintln(os.Stderr, "Missing required flags: -f -u -p")
		fs.Usage()
...
func parseShadow(ctx *Context) State {
	...
	for sc.Scan() {
		...
		if ctx.PwInfo.AlgID == "1" {
			...
			// bcrypt: $2b$cost$salt+hash (salt+hash in tokens[3])
			...
			// sha256/sha512/yescrypt: $id$...$salt$hash
		} else {
			if len(tokens) < 5 {
				ctx.ExitMessage = "ERROR: User hash failed to tokenize"
				return StateError
...
func acceptWorker(ctx *Context) State {
	...
	// match "ip:port" output (remoteAddr already is that; strip brackets for IPv6 if desired)
...
func waitResult(ctx *Context) State {
	...
	result, err := recvMsg(ctx.WorkerConn)
	if err != nil {
		ctx.ExitMessage = fmt.Sprintf("ERROR: Failed to receive result. %v", err)
		return StateError
	}
	...
	// python: ctx.result_return_latency = result_latency["result_return_latency"]
...
func main() {
	...
	// Ctrl+C => cleanup (Python KeyboardInterrupt)
